  ...
  useEffect(() => {
    const loadCollectionData = async () => {
      ...
      try {
        ...
        console.log("Fetched NFTs for collection:", nftData.nfts); // Debug log
  ...
  const renderNFTCard = (nft: NFT) => {
    ...
    return (
      <Card
        ...
        onClick={() =>
          navigate(`/collections/${collectionId}/nfts/${nft.tokenId}`)
        }
      >
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  if (e.currentTarget.nextElementSibling) {
                    (e.currentTarget.nextElementSibling as HTMLElement).style.display = "block"; // Show fallback
                  }
                }}
              />
            ) : (
              <div
                className="h-40 w-40 rounded-lg bg-gray-100 flex items-center justify-center"
                style={{ display: collection.imageUrl ? "none" : "block" }}
              >
                <Info className="h-12 w-12 text-gray-400" />
              </div>
            )}
